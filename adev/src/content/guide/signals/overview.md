...
Only the signals, which are actually read during the derivation, are tracked. For example, in this `computed` the `count` signal is only read if the `showCount` signal is true:
